...
const Ingredients = ({addQuantity, addIngredient, index}) => {
    return (
        <div className='ingredients-input-container'>
            <div>
           <label htmlFor="quantity">Quantity</label>
            <input name="quantity" type="text" onChange={(e) => addQuantity(e, index)}/>    
            </div>

            <div>
            <label htmlFor="ingredient">Ingredient</label>
            <input name="ingredient" type="text" onChange={(e) => addIngredient(e, index)}/>
           </div>
            </div>
    )
...
const Input = (props) => {
    ...
    const submitHandler = (e) => {
        ...
        console.log('submit clicked')
                ...
                axios.post("http://localhost:3015/posts/", recipeInfo)
    ...
    if(loaded){
        ...
        return(
            ...
            <form onSubmit={submitHandler}>
                ...
                <input type="text" id='author' name='author' onChange={inputHandler}/>
               
                <label htmlFor="country">Recipe is from</label>
                <select name="country" value={recipeInfo.country} onChange={inputHandler}>
               
                {countries.map(country => <Option key={country} origin={country}/>)}
              
                </select>
                <label htmlFor="descriptiom">Description</label>
                <textarea name="description" id="description" cols="30" rows="10" onChange={inputHandler} />
               ...
               <input type="text" id="image" name="image" onChange={inputHandler}/>
                ...
                <textarea name="instructions" id="instructions" cols="30" rows="10" onChange={inputHandler} />
